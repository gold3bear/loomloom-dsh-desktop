...
- loading（骨架屏：标题 + 区块头 + 加载文案，**不得**渲染连接引导）；
- `public`：未提供创作者凭据时读取公开市场全量目录，并受最大页数保护；
...
**行的结构**：每行是一个容器，内部两个并列按钮——行主体（调用）与右侧预览 icon。预览按钮**不得**嵌进行主体按钮（非法嵌套交互元素）。

| 操作 | 行为 |
|---|---|
| 点**行主体** | 走 §5.4 的调用流程；消息里说明「我尚未填写输入，请先向我收集必填项」 |
| 点**预览 icon** | 打开预览面板（覆盖层）：版本号、更新时间、创作者、费用、可执行状态 + **可填写的输入表单** |
...
`loomloom_get_run_results` 的 `output.render` 把产物的 `inlineText` 渲染成结构化内容（实现见 `src/result-presentation.ts`，纯函数、可单测）：

| 输入形状 | 输出 |
|---|---|
| 对象数组（扁平） | **Markdown 表格**；列 = 键的并集（上限 8），行上限 20 |
| 单对象 | 两列键值表；嵌套对象按点路径展开（深度上限 2），更深的显示 `{…}`，数组显示 `[N items]` |
| 标量数组 | 单列表格 |
| 非 JSON 文本 | fenced code block |
| 裸标量 | fenced code block（不做单格表格） |

对话渲染器支持 GFM 表格（`micromark-extension-gfm-table` 在客户端依赖内），所以走普通 text 内容块即可，**不需要新增 ContentBlock 类型、也不需要改上游**。
...
- 默认公开市场升级时忽略旧版无来源标识的缓存；缓存按 API 地址、市场来源、固定列表或创作者身份摘要隔离，不保存创作者明文 Key；
- 公开市场无需登录，读取完整分页；首次读取失败仍报告已配置的公开来源与网络错误，不显示“此构建尚未配置店面”；
- 同来源成功快照可跨重启复用，失败时保留同来源旧数据；来源变化时禁止回退到其他来源的数据；
- 列表成功、空列表、加载中、网络失败；
